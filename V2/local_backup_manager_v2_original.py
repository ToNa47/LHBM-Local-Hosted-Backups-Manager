...
Cara Track:
    ...
    3. Atur durasi pemantauan (interval)
...
class BackupJob:
    ...
    interval: int = 30       # detik antar-cek (durasi tracking)
...
class JobRunner(threading.Thread):
    """
    Memantau `job.source` setiap `job.interval` detik. Begitu ada file
    ...
    def run(self):
        self.log(f"[{self.job.name}] Tracking dimulai (setiap {self.job.interval}s)")
...
class App:
    ...
    def _build_ui(self):
        ...
        headers = {
            ...
            "interval": "Durasi(s)", "status": "Status", "last": "Update Terakhir",
        }
...
class JobDialog:
    def __init__(self, parent, job: BackupJob = None):
        ...
        ttk.Label(self.top, text="Durasi tracking / interval cek (detik):").grid(row=r, column=0, sticky="w", **pad)
    ...
    def _save(self, existing_job):
        ...
        try:
            ...
            messagebox.showerror("Error", "Durasi harus > 0 dan Retention harus angka >= 0")
            return
